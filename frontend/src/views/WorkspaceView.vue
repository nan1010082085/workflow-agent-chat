...
<template>
  <div class="workspace">
    <header class="topbar">
      <div><strong>对话</strong><span class="subtle">直接使用平台模型，或选择一个已发布的专用能力</span></div>
    ...
    <main class="conversation" :class="{ empty: !hasMessages }">
      <section v-if="!hasMessages" class="welcome">
        ...
        <p v-if="modelStore.error" class="error-copy">{{ modelStore.error }}</p>
      </section>
...
.topbar { min-height:68px; display:flex; justify-content:space-between; align-items:center; gap:16px; padding:12px 32px; border-bottom:1px solid var(--c-border); background:var(--c-surface); }
...
.conversation { flex:1; display:flex; flex-direction:column; min-height:0; overflow:hidden; }.welcome { margin:auto auto 24px; width:min(720px,calc(100% - 48px)); text-align:center; }.welcome-mark { color:var(--c-accent); font-size:28px; }.welcome h1 { margin:12px 0 8px; font-size:30px; }.welcome p { margin:0 0 20px; color:var(--c-text-secondary); }.welcome .model-picker { margin:auto; }.error-copy { color:var(--c-danger)!important; font-size:12px; margin-top:12px!important; }.global-error { position:fixed; bottom:16px; left:50%; transform:translateX(-50%); background:var(--c-danger); color:white; padding:9px 16px; border-radius:6px; font-size:13px; }
@media(max-width:767px){.topbar{padding:12px 16px}.subtle{display:none}.workflow-panel{width:calc(100% - 32px)}.welcome{width:calc(100% - 32px)}.welcome h1{font-size:26px}}
</style>
