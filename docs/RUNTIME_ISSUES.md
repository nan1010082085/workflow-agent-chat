...
> 本文档记录 Workflow Agent Chat 在开发过程中遇到的、**涉及底层支撑项目（Schema Platform AI Runtime / AI Platform / server）**的契约、接口、凭证与能力待确认问题。
>
> **原则**：Chat 项目只消费 Runtime 能力，不擅自修改底层支撑项目的代码或接口。以下问题需要与 Runtime / 平台团队对齐后，Chat 侧再据此调整 adapter 实现。
