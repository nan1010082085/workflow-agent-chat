...
**产品职责**：定义用户任务、能力目录、会话规则、状态语言、权限边界和验收标准。产品回答“用户要完成什么、什么时候算完成、异常后下一步是什么”，不把 Workflow、Run、模型供应商当成用户任务。

**前端职责**：把一个任务从开始到完成呈现清楚。负责工作台布局、助手选择、消息输入、状态展示、确认操作、结果操作和响应式体验；不决定租户权限、不保存密钥、不直接访问 Runtime、不自行选择模型或伪造执行状态。

**服务端职责**：作为唯一业务边界。负责认证、租户隔离、助手目录、会话与消息持久化、幂等与并发、Runtime/模型 BFF、错误归因和状态映射；不承载 UI 状态，不把底层异常吞成空列表，不把 Workflow 图复制到 Chat。
